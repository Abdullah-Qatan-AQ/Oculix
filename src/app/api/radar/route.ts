...
/**
 * OCULIX — Internet Outage Detection (IODA)
...
export async function GET() {
  try {
    ...
    const res = await fetch(url, {
      ...
      cache: 'no-store',
      headers: { 'User-Agent': 'OCULIX/4.2', 'Accept': 'application/json' },
    });

    console.log('[OCULIX] IODA response status:', res.status);
    ...
    console.error('[OCULIX] IODA fetch error:', error);
