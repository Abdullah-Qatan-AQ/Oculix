...
async function getOpenSkyToken(): Promise<string | null> {
  ...
  try {
    ...
    if (!res.ok) { console.warn('[OCULIX] OpenSky token failed:', res.status); return null; }
    const data = await res.json();
    if (!data.access_token) {
      console.warn('[OCULIX] OpenSky token response missing access_token');
      return null;
    }
    ...
    console.warn('[OCULIX] OpenSky token error:', e);
    return null;
  }
}
...
export async function GET() {
  ...
  fetchPromise = (async () => {
    ...
    if (milRes.status === 'fulfilled') {
      if (milRes.value.ok) {
        try {
          ...
          console.warn('[OCULIX] adsb.fi mil parse error:', e);
        }
      } else {
        console.warn('[OCULIX] adsb.fi mil feed returned', milRes.value.status);
    ...
    if (osRes.status === 'fulfilled') {
      if (osRes.value.status === 429) {
        ...
        console.warn('[OCULIX] OpenSky 429 — cooling down 15 min');
        ...
        try {
          ...
          console.warn('[OCULIX] OpenSky parse error:', e);
        }
      } else {
        // A rejected token surfaces here as a 401 — previously discarded silently.
        console.warn('[OCULIX] OpenSky returned', osRes.value.status);
        await osRes.value.body?.cancel();
      }
    }
    ...
    if (!openSkyWorked) {
      source = 'regional';
      console.warn('[OCULIX] no OpenSky snapshot — falling back to adsb.fi regional sweep');
      ...
      if (allRaw.length === 0) {
        console.error(
          '[OCULIX] every flight provider returned zero aircraft — ' +
  ...
  try {
    ...
    console.error('[OCULIX] Flight fetch error:', error);
    fetchPromise = null;
    // Stale-cache fallback: return last known good data instead of blank map
    if (cachedData) {
      console.warn('[OCULIX] Returning stale flight cache as fallback');
