...
import { BellRing, Bookmark, Check, Plus, Trash2, X } from 'lucide-react';
...
type Watchlist = { id: string; name: string; focus: string; rule: RuleId; createdAt: string };
...
function isWatchlist(value: unknown): value is Watchlist {
  ...
  return typeof item.id === 'string' && typeof item.name === 'string' && typeof item.focus === 'string' && typeof item.createdAt === 'string' && typeof item.rule === 'string' && RULE_IDS.includes(item.rule as RuleId);
}
...
export default function WatchlistPanel({ open, onClose, language, metrics }: WatchlistPanelProps) {
  ...
  useEffect(() => { if (open) setItems(readWatchlists()); }, [open]);
  const save = (next: Watchlist[]) => { const bounded = next.slice(0, 20); setItems(bounded); writeWatchlists(bounded); };
  const add = () => { if (!name.trim()) return; save([{ id: `WL-${Date.now().toString(36).toUpperCase()}`, name: name.trim(), focus: focus.trim(), rule, createdAt: new Date().toISOString() }, ...items].slice(0, 20)); setName(''); setFocus(''); };
  ...
  return <aside className="oculix-watchlist-panel" dir={language === 'ar' ? 'rtl' : 'ltr'} aria-label={t.title}>
    ...
    <section className="oculix-watchlist-list"><h3>{t.saved} <span>{items.length}</span></h3>{items.length === 0 ? <p>{t.empty}</p> : items.map(item => <article key={item.id}><div><strong>{item.name}</strong><small>{item.focus || '—'} · {ruleLabel(item.rule)} · {activeSignals[item.rule] ? t.active : t.local}</small></div><button type="button" onClick={() => save(items.filter(entry => entry.id !== item.id))} aria-label={t.remove} title={t.remove}><Trash2 size={14} /></button></article>)}</section>
