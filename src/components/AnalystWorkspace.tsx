...
import { Archive, BookmarkPlus, Check, Clock3, FileText, GitBranch, Link2, LockKeyhole, Save, ShieldAlert, Sparkles, X } from 'lucide-react';
...
export default function AnalystWorkspace({ open, onClose, language, metrics }: AnalystWorkspaceProps) {
  ...
  return (
    <aside className="oculix-analyst-workspace" dir={language === 'ar' ? 'rtl' : 'ltr'} aria-label={t.title}>
      <header className="oculix-analyst-header"><div><div className="oculix-kicker"><Sparkles size={13} /> ANALYST MODE</div><h2>{t.title}</h2><p>{t.subtitle}</p></div><button type="button" onClick={onClose} aria-label={t.close} title={t.close}><X size={16} /></button></header>
