...
function OculixMap({ data, activeLayers, onEntityClick, onMouseCoords, onRightClick, onViewStateChange, flyToLocation, projection = 'globe', mapStyle = 'dark', sweepData, scanTargets = [], demoMode = false, theme = 'core', drawnPolygons = [], arcgisLayers = [], drawMode = null, onDrawComplete, onDrawProgress, onDrawCancel, drawCommand = null, onMapCenter, route = null, userLocation = null, followUser = false, onFollowInterrupt, navigating = false, aircraftAirports = {} }: OculixMapProps) {
  ...
  useEffect(() => {
    ...
    map.on('load', () => {
      ...
      // ══ OCULIX SDK — Lattice Intelligence Mesh ══
      ...
      if (process.env.NODE_ENV === 'development') (window as any).__oculixMap = map;
    });
      ...
      map.on('click', layer, e => {
        ...
        popup([coords.lng, coords.lat], `<div style="${pStyle}border:1px solid ${domainColor}40;">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;font-size:9px;margin-bottom:8px;">
            ...
            <div><span style="color:#5C5A54;">SOURCE</span><br/><a href="${urlSafe(srcUrl)}" target="_blank" style="color:${domainColor};text-decoration:underline;cursor:pointer;">${htmlEsc(p.source || 'OCULIX')}</a></div>
          </div>
  ...
  // ══ OCULIX SDK — Lattice Sensor Mesh ══
  ...
  useEffect(() => {
    ...
    try {
      if (enabled) {
        ...
        if (!map.getSource('oculix-buildings')) {
          map.addSource('oculix-buildings', {
            type: 'vector',
        ...
        if (!map.getLayer('oculix-3d-buildings')) {
          map.addLayer({
            id: 'oculix-3d-buildings',
            source: 'oculix-buildings',
        ...
        // ── DISABLE 3D ──
        if (map.getLayer('oculix-3d-buildings')) map.removeLayer('oculix-3d-buildings');
      }
    } catch (e) {
      console.warn('[OCULIX] 3D terrain toggle error:', e);
    }
