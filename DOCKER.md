# Self-Hosting OCULIX with Docker

OCULIX ships as a self-contained Next.js standalone build. This guide covers
running it with Docker / Docker Compose, deploying it as a [CasaOS](https://casaos.io)
app, and configuring the optional API keys.

> **TL;DR:** OCULIX runs fully **without any API keys**. All core feeds
...
- **`build:`** — compose builds the image locally from the `Dockerfile`, so
  ...
  instead, add `image: ghcr.io/your-github-account/oculix:latest` to the `oculix`
  service and drop the `build:` block.
- **`env_file: .env` (`required: false`)** — if a `.env` file exists its
  values are injected into the container; if it's missing, OCULIX still starts
  with the keyless feeds.
- **`ports: ${OCULIX_PORT:-3000}:3000`** — the web UI. The container always
  listens on 3000; the published **host** port is `OCULIX_PORT` (default
  `3000`). Set `OCULIX_PORT` in `.env` to remap it, e.g. `OCULIX_PORT=3005`
...
run OCULIX without building anything:

```bash
docker pull ghcr.io/your-github-account/oculix:latest   # or a pinned tag, e.g. :0.1.0
docker run -d --name oculix \
  -p 3005:3000 --env-file .env --restart unless-stopped \
  ghcr.io/your-github-account/oculix:latest
```
...
```bash
docker build -t oculix:latest .
docker run -d --name oculix -p 3000:3000 --env-file .env --restart unless-stopped oculix:latest
```
...
3. OCULIX appears on the dashboard with its icon, reachable on host port
   `3000` (or whatever `OCULIX_PORT` you set in `.env`).
...
> directly, either build/tag `oculix:latest` first
> (`docker build -t oculix:latest /path/to/oculix`) or replace the `build:`
> block with `image: ghcr.io/your-github-account/oculix:latest`.
...
| `SCANNER_KEY` | Shared secret; **must equal the backend's `OCULIX_KEY`** | RECON toolkit |

Without `SCANNER_URL`/`SCANNER_KEY` the RECON endpoints return `503` and the
rest of OCULIX works normally. Generate a key with `openssl rand -hex 32`.
...
|----------|---------|---------|
| `OCULIX_TELEGRAM_CHANNELS` | Comma-separated list of public Telegram channel usernames (no `@`) to scrape for the **Telegram OSINT** map layer. Overrides the curated default set. | `osintdefender,insiderpaper,aljazeeraenglish,nexta_live,war_monitor` |
| `OCULIX_PORT` | Host port the compose file publishes (container itself always listens on 3000). | `3000` |
